...
func (c *COSClient) CreateBucket() {
	...
	if err != nil {
		log.Println(fmt.Sprintf("create bucket err = %v", err))
		return
	}
}

// Buckets bucket列表
func (c *COSClient) Buckets() []Bucket {
	s, _, err := c.client.Service.Get(context.Background())
	if err != nil {
		log.Println(fmt.Sprintf("bucket err = %v", err))
...
func (c *COSClient) List(path string, next string, limit int) BucketResult {
	...
	if err != nil {
		log.Println(fmt.Sprintf("Get bucket list err = %v", err))
		return BucketResult{}
	}
	...
	return BucketResult{Content: data, Dirs: dirs}
}
